...
func findArg(line string) (string, error) {
	...
	case strings.Contains(line, "/"):
		return "/", nil
	default:
		return "", fmt.Errorf("Выдача паники, т.к. строка не является математической операцией")
	}
}
...
func getNumsAndType(line string, op string) (a, b int, rom bool, err error) {
	...
	if len(nums) > 2 {
		return a, b, rom, fmt.Errorf("Выдача паники, т.к. формат математической операции не удовлетворяет заданию - два операнда и  один оператор")
	}
	...
	if romOne != romTwo {
		return a, b, rom, fmt.Errorf("Выдача паники, так как используються одновременно разные системны счисления")
